...
export function SectionCardSmall({
  context,
  element,
  actions,
  dnd,
  flags,
}: SectionCardSmallProps) {
  const navigate = useNavigate();
  const [isDuplicating, setIsDuplicating] = useState(false);

  const isHeading = (e: ElementLike): e is HeadingElement =>
    e.kind === 'heading';
  const isSection = (e: ElementLike): e is SectionElement =>
    e.kind === 'section';

  const showCreateVersion = (origin?: Origin) =>
    origin === 'BILANPLUME' || origin === 'COMMUNITY';
  ...
  const coverSrc = useMemo(() => {
    if (isSection(element)) {
      return (
        categories.find((c) => c.id === element.type)?.image ??
        '/bilan-type.png'
      );
    }
  ...
  const clickAction: 'preview' | 'add' | 'none' = useMemo(() => {
    ...
    if (context === 'library')
      return actions?.onPreview ? 'preview' : actions?.onAdd ? 'add' : 'none';
    return 'none';
  ...
  return (
    <div
      ...
      onDragStart={(e) =>
        dnd?.onDragStart && index != null && dnd.onDragStart(e, index)
      }
      ...
      className={`group flex items-stretch gap-0 border border-wood-200 rounded-xl overflow-hidden bg-white transition-all ${
        context === 'library' && clickAction !== 'none'
    ...
    >
      ...
      {/* Right content */}
      <div
        className={`flex-1 ${context === 'library' ? 'p-2 md:p-2' : 'p-3 md:p-3'} flex flex-col`}
      >
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-start gap-2 min-w-0">
            ...
            <div className="min-w-0">
              {isHeading(element) ? (
                <>
                  <div className="mb-2">
                    <Badge className="bg-wood-200 text-wood-600 border-wood-200">
                      Titre
                    </Badge>
                  </div>
                  {context === 'builder' ? (
                    <Input
                      ...
                      placeholder="Titre"
                      onChange={(e) =>
                        actions?.onRenameHeading &&
                        index != null &&
                        actions.onRenameHeading(index, e.target.value)
                      }
          ...
          <div className="flex items-center gap-1 shrink-0">
            {isSection(element) &&
              showCreateVersion(element.origin) &&
              !flags?.hideDuplicate && (
                <Button
                  className="gap-2"
                  variant="ghost"
            ...
            {!isHeading(element) &&
              (context === 'library' ? !flags?.hideOpenAction : true) && (
                <Button
                  variant="ghost"
                  size="sm"
            ...
            {context === 'builder' && (
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (actions?.onRemove && index != null)
        ...
        {isSection(element) && (
          <>
            <div
              className={`${context === 'library' ? 'mt-3' : 'mt-2'} flex items-center gap-2`}
            >
              <Badge
                className={
            ...
            {element.description && (
              <p
                className={`text-xs text-muted-foreground line-clamp-2 ${context === 'library' ? 'mt-2' : 'mt-1'}`}
              >
                {element.description}
              </p>
            )}
