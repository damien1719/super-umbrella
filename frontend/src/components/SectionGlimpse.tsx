...
export default function SectionGlimpse({ title, questions = [], loading = false }: SectionGlimpseProps) {
  ...
  const renderQuestions = useMemo(() => (questions as unknown as RenderQuestion[]) ?? [], [questions]);

  return (
    <div className="flex h-full w-full flex-col">
      <div className="px-4 py-3 bg-white flex items-center gap-4">
        <h3 className="text-lg font-semibold flex-1 truncate" title={title}>{title ?? 'Aperçu de section'}</h3>
      ...
      <div className="flex-1 min-h-0">
        {loading ? (
          <div className="h-full w-full flex items-center justify-center text-gray-500">Chargement…</div>
        ) : tab === 'edition' ? (
              ...
              <div className="h-full overflow-y-auto p-2">
                {selectedId ? (
                  <QuestionList
                    ...
                    onPatch={(_id, _partial) => { /* read-only */ }}
                    onReorder={(_from, _to) => { /* read-only */ }}
                    onDuplicate={(_id) => { /* read-only */ }}
                    onDelete={(_id) => { /* read-only */ }}
                    onAddAfter={(_id) => { /* read-only */ }}
                    isReadOnly={true}
                  />
                ) : (
